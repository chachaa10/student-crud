...
/**
 * Generates HTML for a student row
 ...
 * @returns {Promise<void>} - Resolves when student data is loaded
 */
...
function validateInput(inputField, errorElement) {
  ...
  switch (fieldName) {
    ...
    case 'mobile_number':
      ...
      else if (!/^09[0-9]{9}$/.test(value)) errorMessage = 'Invalid Philippine mobile number (ex. 09123456789)';
      break;
    case 'street_address':
...
function addTableARIA() {
  try {
    ...
    document.querySelectorAll('th[scope=row]').forEach((rowHeader) => rowHeader.setAttribute('role', 'rowheader'));
  } catch (e) {
