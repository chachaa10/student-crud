...
function validateInput(inputField, errorElement) {
  ...
  switch (fieldName) {
    ...
    case 'mobile_number':
      ...
      else if (!/^09[0-9]{9}$/.test(value)) errorMessage = 'Invalid Philippine mobile number (eg. 09123456789)';
      break;
    case 'street_address':
    ...
    case 'student_id':
      ...
      else if (value.length < 8 || !/^\d{8}$/.test(value)) errorMessage = 'Invalid student ID format (ex. 20251234)';
      break;
    case 'password':
      if (!value || value.length < 6 || !/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)[A-Za-z\d]{6,}$/.test(value))
        errorMessage = 'Password must contain at least: 6 characters, 1 uppercase, 1 lowercase, and 1 number';
      break;
    case 'gender':
...
streetAddressInput.addEventListener('blur', () => validateInput(streetAddressInput, streetAddressError));
...
mobileNumberInput.addEventListener('blur', () => validateInput(mobileNumberInput, mobileNumberError));
